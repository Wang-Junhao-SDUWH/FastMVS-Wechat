...
data_dir = os.path.abspath("./Get_pics")
...
print(
    "----------4. Do Incremental/Sequential reconstruction----------")  # set manually the initial pair to avoid the prompt question
...
pRecons = subprocess.Popen(
    [os.path.join(OPENMVG_SFM_BIN, "openMVG_main_openMVG2PMVS"), "-i", reconstruction_dir + "/sfm_data.bin", "-o",
     reconstruction_dir])
pRecons.wait()
...
print("----------8. pmvs2----------")
pRecons = subprocess.Popen([os.path.join(PMVS_BIN, "pmvs2"), reconstruction_dir + "/PMVS/",
                            "pmvs_options.txt"])  # 注：不要修改pmvs_options.txt文件名
pRecons.wait()
