    ...
    with open(path_to_model_file, "rb") as fid:
        ...
        for image_index in range(num_reg_images):
            ...
            while current_char != b"\x00":   # look for the ASCII 0 entry
                ...
                current_char = read_next_bytes(fid, 1, "c")[0]
            print("img index : ", image_id, "  img name : ",image_name)
            imgs_dic[image_id - 1] = image_name[:-4]
...
def read_model(path, ext):
    if ext == ".txt":
        ...
        images, imgs_dic = read_images_binary(os.path.join(path, "images" + ext))
        points3D = read_points3d_binary(os.path.join(path, "points3D") + ext)
    return cameras, images, points3D, imgs_dic
...
if __name__ == '__main__':
    ...
    # 郡浩：read_model 第二个参数改成'.bin'
    cameras, images, points3d, imgs_dic = read_model(model_dir, '.bin')
    ...
    for i in range(num_images):
        with open(os.path.join(cam_dir, '{}.txt'.format(imgs_dic[i])), 'w') as f:
            f.write('extrinsic\n')
            for j in range(4):
                for k in range(4):
